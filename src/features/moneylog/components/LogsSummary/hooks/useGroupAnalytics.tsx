...
interface GroupAnalytics {
  ...
  userRankings: Map<
    Currency,
    Array<{ userId: string; userName: string; dailyAverage: number; weeklyAverage: number }>
  >;
  groupTotals: Map<Currency, number>;
  // Mean of each participant's own daily/weekly average, over members who posted in
  // that currency — "what a typical person here spends", as distinct from groupTotals
  // (the combined total) or currencyPercentiles (their distribution's shape).
...
const calculateUserProfile = (
  ...
  const dailyTotals = new Map<string, Map<Currency, number>>();
  const weeklyTotals = new Map<string, Map<Currency, number>>();
  ...
  currencies.forEach((currency) => {
    ...
    const activeWeeks = [...weeklyTotals.values()].filter((weekMap) =>
      weekMap.has(currency),
    ).length;

    // Count days with activity for this currency
    const activeDays = [...dailyTotals.values()].filter((dayMap) => dayMap.has(currency)).length;
...
export const calculateGroupAnalytics = (
  ...
  const userProfiles: UserSpendingProfile[] = groupMembers.map((member) => {
    ...
    return calculateUserProfile(member.id, member.displayName, userPosts);
  });
  ...
  const perPersonDailyAverages = new Map<Currency, number>();
  const perPersonWeeklyAverages = new Map<Currency, number>();
  ...
  allCurrencies.forEach((currency) => {
    ...
    const groupTotal = userProfiles.reduce(
      (sum, profile) => sum + (profile.totalSpent.get(currency) || 0),
      0,
    );
    groupTotals.set(currency, groupTotal);

    // Per-person average: the mean across the same participants used for rankings/
    // percentiles above, so all three stay consistent about who counts as "a person".
    perPersonDailyAverages.set(currency, mean(rankings.map((r) => r.dailyAverage)));
    perPersonWeeklyAverages.set(currency, mean(rankings.map((r) => r.weeklyAverage)));
  });
